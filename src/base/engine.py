...
class BaseEngine():
    def __init__(self, device, model, dataloader, scaler, sampler, loss_fn, lrate, optimizer, \
                 scheduler, clip_grad_value, max_epochs, patience, log_dir, logger, seed, run_id=''):
        super().__init__()
        self._device = device
        self.model = model
        ...
        self._logger = logger
        self._seed = seed
        self._run_id = run_id  # Unique identifier for parallel runs
    ...
    def save_model(self, save_path):
        ...
        if self._run_id:
            filename = 'final_model_s{}_{}.pt'.format(self._seed, self._run_id)
        else:
